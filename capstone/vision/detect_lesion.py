...
def detect_and_log_grape_properties(img): 
    ...
    for cnt in contours: 
        ...
        if circularity > circularity_threshold: 
            ...
            cv2.line(original, (x+int(w/2), y+int(h/4)), (x+int(w/2), y+int(3*h/4)), 255) # Vertical Line
            cv2.line(original, (x+int(w/4), y+int(h/2)), (x+int(3*w/4), y+int(h/2)), 255) # Horizontal Line
            ...
            if len(all_h) > 0: 
                ...
                print(f"Detected Grape Color Ranges (HSV):") 
                print(f"Hue: {h_range[0]:.2f} - {h_range[1]:.2f}")  
                print(f"Saturation: {s_range[0]:.2f} - {s_range[1]:.2f}")
                print(f"Value: {v_range[0]:.2f} - {v_range[1]:.2f}") 
            else: 
